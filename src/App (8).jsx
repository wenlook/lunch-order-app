...
export default function FoodOrderApp() {
  ...
  const loadCore = useCallback(async () => {
    const [menuVal, qrVal, blocklistVal] = await Promise.all([
...
function UserFlow({ menu, qr, blocklist, onBack, onRefreshMenu, onDone }) {
  ...
  async function handleSubmitOrder() {
    ...
    // while they were mid-order.
    const freshBlocklistVal = await storageGet("admin:blocklist");
    ...
    const blockedNow = freshBlocklist.some((n) => namesMatch(name, n));
    if (blockedNow) {
      setSubmitting(false);
      setClosedMessage(
        ...
        <div className="px-6 pt-8">
          ...
          <div className="flex items-start justify-between mt-6 mb-1">
            ...
            <button
              ...
              className="text-xs font-semibold underline shrink-0 mt-1.5"
              style={{ color: "#1F5F5B" }}
            >
...
function OrdersPanel() {
  ...
  async function blockName(rawName) {
    const trimmed = rawName.trim();
    if (!trimmed) return;
    const val = await storageGet("admin:blocklist");
    let current = [];
    if (val) {
      try {
...
function BlockedUsersPanel() {
  ...
  async function addBlocked() {
    ...
    await persistBlocklist([...blocked, trimmed]);
    setNewName("");
  }
  ...
  return (
    <div>
      <p className="text-sm text-[#7A7166] mb-5">
        ...
        Matching is a bit flexible (catches different spacing/capitalization,
        or a partial name like "Jun Yang" for "Gan Jun Yang") — but if a
        colleague might type an English name one time and a Chinese name
        another, add both as separate entries below.
      </p>
      ...
      {loading ? (
        ...
        <p className="text-sm text-[#B3A992]">Nobody is blocked right now.</p>
      ) : (
        <div
          className="rounded-lg border p-4"
          style={{ borderColor: "#DCD3C2", background: "#FFFFFF" }}
