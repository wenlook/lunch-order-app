...
function BottomNav({ active, onNavigate, cartCount }) {
  ...
  return (
    <div
      ...
      style={{ background: "#FFFFFF", borderTop: "1px solid #DCD3C2" }}
    >
...
function UserFlow({ menu, qr, blocklist, blockedDevices, onBack, onRefreshMenu, onDone }) {
  ...
  async function loadChatMessages() {
    setChatLoading(true);
    const keys = await storageList(`chat:${todayStr()}:`);
    const results = await Promise.all(
      keys.map(async (k) => {
        const val = await storageGet(k);
        if (!val) return null;
    ...
    setChatMessages(results.filter(Boolean).sort((a, b) => b.createdAt - a.createdAt));
    setChatLoading(false);
    setChatLoaded(true);
  }

  function handleChatImageFile(e) {
  ...
  async function sendChatMessage() {
    ...
    const msg = {
      ...
      text: chatText.trim(),
      imageDataUrl: chatImage,
      createdAt: Date.now(),
    };
    await storageSet(`chat:${msg.date}:${msg.id}`, JSON.stringify(msg));
    setChatMessages((prev) => [{ key: `chat:${msg.date}:${msg.id}`, ...msg }, ...prev]);
    setChatText("");
    setChatImage("");
  ...
  async function loadMyStatus() {
    ...
    setMyOrders(
      results
        .filter(Boolean)
        .filter((o) => namesMatch(name, o.name))
  ...
  function handleNavigate(dest) {
    ...
    if (dest === "chat" && !chatLoaded) loadChatMessages();
    if (dest === "status") loadMyStatus();
  }

  // Poll for new chat messages while the chat screen is open, so
  // colleagues see new messages/photos without needing to back out and
  // reopen the chat.
  useEffect(() => {
    if (step !== "chat") return;
  ...
  if (step === "others") {
    ...
    return (
      <div className="px-6 pt-8 pb-24 min-h-screen flex flex-col">
        ...
        {othersLoading ? (
          ...
          <p className="text-sm text-[#B3A992] text-center py-10">No orders yet today.</p>
        ) : (
          <>
            <div
                ...
                <ResponsiveContainer width="100%" height={Math.max(160, chartData.length * 42)}>
                  ...
                  >
                    ...
                    <Tooltip
                      ...
                      contentStyle={{ fontSize: 12, borderRadius: 8, borderColor: "#DCD3C2" }}
                    />
  ...
  if (step === "chat") {
    return (
      <div className="px-6 pt-8 pb-24 min-h-screen flex flex-col">
        <h2 className="fo-serif text-2xl mt-2 mb-1">Team chat</h2>
        <p className="text-sm text-[#7A7166] mb-5">
          {prettyDate(todayStr())} · share food pics &amp; chat with the team
        </p>
        ...
        <div className="flex-1 overflow-y-auto mb-4">
          {chatLoading && chatMessages.length === 0 ? (
            ...
            <p className="text-sm text-[#B3A992] text-center py-10">
              No messages yet — say hi or share a photo!
            </p>
          ) : (
              ...
              {chatMessages.map((m) => (
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm font-semibold fo-serif">{m.name}</span>
        ...
        >
          ...
          <div className="flex items-center gap-2">
            <label
              className="shrink-0 w-10 h-10 rounded-md border flex items-center justify-center cursor-pointer"
              style={{ borderColor: "#DCD3C2" }}
            >
              <ImagePlus className="w-4 h-4" style={{ color: "#7A7166" }} />
              <input type="file" accept="image/*" onChange={handleChatImageFile} className="hidden" />
            </label>
            <input
              value={chatText}
            ...
            <button
              ...
              disabled={chatSending || (!chatText.trim() && !chatImage)}
              className="shrink-0 w-10 h-10 rounded-md flex items-center justify-center disabled:opacity-40"
              style={{ background: "#1F5F5B", color: "#FAF6EE" }}
            >
              {chatSending ? (
  ...
  if (step === "status") {
    return (
      <div className="px-6 pt-8 pb-24 min-h-screen flex flex-col">
        <h2 className="fo-serif text-2xl mt-2 mb-1">Your orders today</h2>
        <p className="text-sm text-[#7A7166] mb-5">{prettyDate(todayStr())}</p>

        {myStatusLoading ? (
          <Spinner label="Checking your orders…" />
        ) : myOrders.length === 0 ? (
          <p className="text-sm text-[#B3A992] text-center py-10">
            You haven't ordered today yet — tap Menu below to get started.
          </p>
        ) : (
          <div className="flex flex-col gap-3">
            {myOrders.map((o) => (
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  ...
                  <Stamp tone={toneForStatus(o.status)}>{statusLabel(o.status)}</Stamp>
                </div>
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <div className="px-6 pt-8">
        ...
        <div className="flex gap-1 border-b overflow-x-auto" style={{ borderColor: "#DCD3C2" }}>
          {[
            { id: "menu", label: "Menu" },
            { id: "qr", label: "QR" },
              ...
              {confirmingId === order.key ? (
                ...
                <div className="flex items-center gap-3 mt-2">
                  <button
                    ...
                    className="text-xs text-[#B3A992] underline flex items-center gap-1"
                  >
                    <Trash2 className="w-3 h-3" />
                    Delete order
                  </button>
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <p className="text-sm text-[#B3A992] mb-5">No names blocked right now.</p>
      ) : (
        <div
          className="rounded-lg border p-4 mb-5"
          style={{ borderColor: "#DCD3C2", background: "#FFFFFF" }}
            ...
            <div key={n}>
              <div className="flex items-center justify-between py-2">
                ...
                <button
                  ...
                  style={{ borderColor: "#DCD3C2", color: "#1F5F5B" }}
                >
                  Unblock
                </button>
              </div>
      ...
      <p className="text-xs text-[#7A7166] mb-3">
      ...
      {loading ? null : blockedDevices.length === 0 ? (
        <p className="text-sm text-[#B3A992]">No devices blocked right now.</p>
      ) : (
        <div
          className="rounded-lg border p-4"
          style={{ borderColor: "#DCD3C2", background: "#FFFFFF" }}
            ...
            <div key={d.deviceId}>
              <div className="flex items-center justify-between py-2">
                <div>
                  ...
                  <div className="text-[10px] text-[#B3A992]">
                    Blocked{" "}
                    {d.blockedAt
...
function ChatModerationPanel() {
  ...
  const loadMessages = useCallback(async (d) => {
    setLoading(true);
    const keys = await storageList(`chat:${d}:`);
    const results = await Promise.all(
      keys.map(async (k) => {
        const val = await storageGet(k);
        if (!val) return null;
    ...
    setMessages(results.filter(Boolean).sort((a, b) => b.createdAt - a.createdAt));
    setLoading(false);
  }, []);
  ...
  async function handleDeleteMessage(key) {
    setBusyKey(key);
  ...
  return (
    <div>
      <p className="text-sm text-[#7A7166] mb-5">
        Everyone with the app link can see this chat and post photos — no
        admin approval needed before something appears. Use this tab to
      ...
      {loading ? (
        ...
        <p className="text-sm text-[#B3A992] text-center py-10">
          No messages for {prettyDate(date)} yet.
        </p>
      ) : (
        <div className="flex flex-col gap-3">
          {messages.map((m) => (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <span className="fo-serif text-base">{m.name}</span>
                <span className="text-[11px] text-[#B3A992]">
                  {new Date(m.createdAt).toLocaleTimeString("en-MY", {
                    hour: "2-digit",
                    minute: "2-digit",
                  })}
              ...
              {m.text && <p className="text-sm mb-2">{m.text}</p>}
              {m.imageDataUrl && (
                <button type="button" onClick={() => setZoomedPhoto(m.imageDataUrl)} className="mb-2">
                  <img
                    src={m.imageDataUrl}
                    alt="Shared food photo"
              ...
              {confirmingKey === m.key ? (
                <div
                  ...
                  style={{ borderColor: "#EEE7D8" }}
                >
                  <span className="text-xs text-[#B33A2E]">Delete this message?</span>
                  <div className="flex gap-2">
                    <button
                    ...
                    <button
                      ...
                      onClick={() => handleDeleteMessage(m.key)}
                      className="px-3 py-1 rounded-md text-xs font-semibold flex items-center gap-1"
                      style={{ background: "#B33A2E", color: "#FAF6EE" }}
                    >
                      {busyKey === m.key ? (
                ...
                <button
                  onClick={() => setConfirmingKey(m.key)}
                  className="text-xs text-[#B3A992] underline flex items-center gap-1"
                >
                  <Trash2 className="w-3 h-3" />
                  Delete
      ...
      {zoomedPhoto && (
        ...
        >
          <img
            src={zoomedPhoto}
            alt="Shared food photo enlarged"
            className="w-full max-w-sm rounded-lg"
