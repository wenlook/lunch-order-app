            ...
            <>
              {dish.optionGroups && dish.optionGroups.length > 0 && (
                <div className="flex flex-col gap-1.5 mt-1.5 mb-1.5">
                  {dish.optionGroups.map((group) => {
                    const selectedId = getSelectedChoiceId(dish, group);
                    return (
                      <div key={group.id}>
                        <div className="text-[9px] uppercase tracking-wide font-semibold text-[var(--text-faint)] mb-1">
                          {group.name}
                        </div>
                        <div className="flex flex-wrap gap-1">
                          {group.choices.map((choice) => {
                            ...
                            return (
                              <button
                                key={choice.id}
                                onClick={() => setOptionChoice(dish.id, group.id, choice.id)}
              ...
              {dish.sizes && dish.sizes.length > 0 ? (
                <div className="flex flex-col gap-1.5 mt-1.5">
                  {dish.sizes.map((size) => {
                    const qty = getCartQty(dish, size.id);
                    return (
                      <div key={size.id} className="flex items-center justify-between gap-1">
                        ...
                        {qty === 0 ? (
                          <button
                            onClick={() => setCartQty(dish, size.id, 1)}
                          ...
                          >
                            <button
                              onClick={() => setCartQty(dish, size.id, qty - 1)}
                ...
                <div className="flex items-center justify-between mt-1.5">
                  ...
                  {getCartQty(dish, null) === 0 ? (
                    <button
                      onClick={() => setCartQty(dish, null, 1)}
                    ...
                    >
                      <button
                        onClick={() => setCartQty(dish, null, getCartQty(dish, null) - 1)}
                        className="w-5 h-5 flex items-center justify-center"
                        style={{ color: "#1F5F5B" }}
                      >
  ...
  if (step === "help") {
    return (
      ...
      <div className="px-6 pt-8 pb-24 min-h-[100dvh] flex flex-col">
        ...
        <p className="text-xs text-[var(--text-faint)] mb-5 text-center">
