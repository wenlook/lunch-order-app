...
function UserFlow({ menu, qr, blocklist, blockedDevices, onBack, onRefreshMenu, onDone }) {
      ...
      <div className="min-h-screen flex flex-col pb-28">
        <div className="px-6 pt-8">
          ...
          <div className="flex items-start justify-between mt-6 mb-1">
            <h2 className="fo-serif text-2xl">Today's menu</h2>
            <div className="flex flex-col items-end gap-1 shrink-0 mt-1.5">
              <button
                onClick={() => {
                  setStep("others");
                  if (!othersLoaded) loadOthersOrders();
                }}
                className="text-xs font-semibold underline"
                style={{ color: "#1F5F5B" }}
              >
                See what others ordered
              </button>
              <button
                onClick={() => {
                  setStep("chat");
  ...
  if (step === "others") {
    ...
    return (
      <div className="px-6 pt-8 pb-10 min-h-screen flex flex-col">
        <BackRow onBack={() => setStep("menu")} label="Back to menu" />
        <h2 className="fo-serif text-2xl mt-6 mb-1">What everyone's ordering</h2>
        <p className="text-sm text-[#7A7166] mb-5">{prettyDate(todayStr())}</p>
  ...
  if (step === "chat") {
    return (
      <div className="px-6 pt-8 pb-6 min-h-screen flex flex-col">
        <BackRow onBack={() => setStep("menu")} label="Back to menu" />
        <h2 className="fo-serif text-2xl mt-6 mb-1">Team chat</h2>
