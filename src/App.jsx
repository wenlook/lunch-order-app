...
function UserFlow({ menu, qr, blocklist, blockedDevices, onBack, onRefreshMenu, onDone }) {
  ...
  async function loadMyStatus() {
    ...
    setMyOrders(results.filter(Boolean).sort((a, b) => b.createdAt - a.createdAt));
  ...
  if (step === "menu") {
    ...
    visibleDishes = [...visibleDishes].sort((a, b) => (b.isMain ? 1 : 0) - (a.isMain ? 1 : 0));
    ...
    const renderDishCard = (dish) => (
      ...
      >
        ...
        {dish.extraName && (
          <div className="text-[10px] text-[#7A7166] italic mb-0.5">{dish.extraName}</div>
        )}
        {dish.desc && <div className="text-[10px] text-[#7A7166] mb-1">{dish.desc}</div>}
    ...
    return (
      <div className="min-h-screen flex flex-col pb-24">
        <div className="px-6 pt-8">
          ...
          <p className="text-sm text-[#7A7166] mb-4">{prettyDate(todayStr())}</p>
        ...
        <div className="px-6 flex gap-3 flex-1">
          ...
          <div className="flex-1">
            {visibleDishes.length === 0 ? (
              <p className="text-sm text-[#B3A992] text-center py-10">No dishes found.</p>
            ) : (
              ...
              <div className="flex flex-col gap-3 mb-5">
                {myHelpRequests.map((h) => (
                  ...
                  >
                    ...
                    {h.orderFood && (
                      <div className="text-xs text-[#7A7166] mb-2">Order: {h.orderFood}</div>
                    )}
                    {h.message && <p className="text-xs text-[#7A7166] mb-2">{h.message}</p>}
                    {h.status === "paid_helping" && (
  ...
  if (step === "status") {
    return (
      <div className="px-6 pt-8 pb-24 min-h-screen flex flex-col">
        ...
        <p className="text-sm text-[#7A7166] mb-5">{prettyDate(todayStr())} · everyone's orders</p>

        {myStatusLoading ? (
          <Spinner label="Loading orders…" />
        ) : myOrders.length === 0 ? (
          <p className="text-sm text-[#B3A992] text-center py-10">
            No orders yet today — tap Menu below to get started.
          </p>
        ) : (
            ...
            {myOrders.map((o) => {
              ...
              return (
                <div
                  ...
                  className="rounded-lg border p-4"
                  style={{
                    borderColor: isMine ? "#1F5F5B" : "#DCD3C2",
                    background: "#FFFFFF",
                  }}
                >
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <div className="fo-serif text-base">
                        {o.name}
                        {isMine && (
                          <span className="text-[10px] font-semibold ml-1.5" style={{ color: "#1F5F5B" }}>
                            (you)
                          </span>
                        )}
                      </div>
        ...
        >
          ...
          {proofUploading ? (
            <div className="flex flex-col items-center py-8">
              ...
              <span className="text-xs text-[#7A7166]">Processing photo…</span>
            </div>
          ) : proofImage ? (
  ...
  return (
    <div>
      ...
      <div className="rounded-lg border p-4 mt-6" style={{ borderColor: "#DCD3C2", background: "#FFFFFF" }}>
        ...
        <div className="flex items-center justify-between rounded-md border p-3 mb-2" style={{ borderColor: "#DCD3C2" }}>
          <div>
            <div className="text-sm font-medium">This dish has size options</div>
            <div className="text-xs text-[#7A7166]">e.g. Small / Big, each with its own price</div>
          </div>
          <button
        ...
        <div className="flex items-center justify-between rounded-md border p-3 mb-3" style={{ borderColor: "#DCD3C2" }}>
          <div>
            <div className="text-sm font-medium">Feature as Main dish</div>
            <div className="text-xs text-[#7A7166]">Shows at the top of the menu for colleagues</div>
          </div>
          <button
          ...
          {orders.map((order) => (
            ...
            >
              <div className="flex items-start justify-between mb-2">
                ...
                <div className="flex flex-col items-end gap-1">
                  ...
                  {order.status === "confirmed" && (
                    <span className="text-[10px] font-semibold text-[#7A7166]">
              ...
              {(order.status === "confirmed" || order.status === "rejected") && (
                <div>
                  <div className="text-[10px] uppercase tracking-wide font-semibold text-[#7A7166] mb-1.5">
                    Paid via / override
                  </div>
        ...
        <div className="flex flex-col gap-3">
          {requests.map((r) => (
            ...
            >
              ...
              {r.message && <p className="text-xs text-[#7A7166] mb-3">{r.message}</p>}
              {r.status === "pending" && (
              ...
              {r.status !== "pending" && (
                <button
                  ...
                  className="text-xs text-[#B3A992] underline mr-3"
                >
                  reset to pending
                </button>
              )}
              <button
                ...
                className="text-xs text-[#B3A992] underline flex items-center gap-1 mt-2"
              >
                <Trash2 className="w-3 h-3" />
                Delete
              </button>
