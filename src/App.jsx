...
function UserFlow({ menu, qr, onBack, onRefreshMenu, onDone }) {
  ...
  const items = Object.entries(cart)
    ...
    .map(([dishId, qty]) => {
      ...
      return dish ? { dishId, name: dish.name, price: dish.price, qty } : null;
    })
    .filter(Boolean);
...
function MenuEditor({ menu, onSaveMenu }) {
  ...
  async function persist(nextDishes, nextOpen) {
    setSaving(true);
    await onSaveMenu({ dishes: nextDishes, isOpen: nextOpen, updatedAt: Date.now() });
    setSaving(false);
  }
