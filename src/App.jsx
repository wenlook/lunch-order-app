  ...
  if (step === "menu") {
    ...
    const renderDishCard = (dish) => (
      ...
      >
        {dish.soldOut && (
          <div
            className="absolute top-2 right-[-28px] rotate-45 px-8 py-0.5 text-[10px] font-bold uppercase tracking-wide"
            style={{ background: "#B33A2E", color: "#FFFFFF" }}
          >
            Sold out
          </div>
  ...
  if (step === "status") {
    return (
      ...
      <div className="px-6 pt-8 pb-24 min-h-[100dvh] flex flex-col">
        ...
        <p className="text-sm text-[var(--text-muted)] mb-5">{prettyDate(todayStr())} · everyone's orders</p>
...
function AdminLogin({ onBack, onSuccess }) {
  ...
  async function handleResetPasscode() {
    setBusy(true);
    await storageDelete("admin:passcode");
    setBusy(false);
    setPass("");
    setConfirmPass("");
          ...
          <div className="mt-4 text-center">
            {confirmingReset ? (
              <div
                ...
                style={{ borderColor: "#B33A2E", background: "#FBEEEC" }}
              >
                <p className="text-xs mb-2" style={{ color: "#B33A2E" }}>
                  This clears the current passcode so anyone can set a new
                  one. Only do this if you're locked out.
                </p>
                <div className="flex gap-2">
                  <button
...
function AdminPanel({ menu, qr, onSaveMenu, onSaveQr, onLock, darkMode, onToggleDarkMode }) {
